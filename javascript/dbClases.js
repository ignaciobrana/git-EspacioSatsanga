...
$(function() {
    ...
    $.ajax({
        ...
        contentType : 'application/json',
        dataType : 'json',
        data: { condicion: 'getEstadoClases_All' }
    }).done(
        //Una vez cargados los estadosClases configuramos la grid para poder utilizar los estados en los select
        function(jsonEstadoClaseList){
            //insertamos como primer elemento uno vacio
            jsonEstadoClaseList.unshift({ _idEstadoClase: '0', _nombre: '' });
    ...
    $.ajax({
        ...
        contentType : 'application/json',
        dataType : 'json',
        data: { condicion: 'getDia_All' }
    }).done(
        function(jsonDiaList){
            //insertamos como primer elemento uno vacio
            jsonDiaList.unshift({ _idDia: '0', _nombre: '' });
    ...
    $('#detailsForm').validate({
        rules: {
            sEmpleado:{ required: true, min:1 },
            sDia:{ required: true, min:1 },
            tHoraInicio: { required: true },
            tHoraFin: { required: true },
            sEstado:{ required: true, min:1 },
            tDescripcion: { required: false, maxlength: 255 }
        },
...
var showDetailsDialog = function(dialogType, clase) {
    ...
    $('#detailsDialog')
        .dialog('option', 'title', dialogType + ' clase')
        .dialog('open');
};

var saveClase = function(clase, isNew) {
    ...
    if (idEstadoClase != '2') {// Si es distinto a Estado Inactivo, entonces...
    ...
    $('#jsGrid').jsGrid({
        ...
        fields: [
            {   
                ...
                title: 'Día', 
                type: 'select', 
                items: db.dia, valueField: '_idDia', textField: '_nombre',
                sorting: false
            },
            { name: '_horaInicio', type: 'text', width: 80, title: 'Hora Inicio', sorting: false, filtering: false },
            { name: '_horaFin', type: 'text', width: 80, title: 'Hora Fin', sorting: false, filtering: false },
            { name: '_nombreEmpleado', type: 'text', width: 80, title: 'Profesor', sorting: true },
            {   
                name: '_estadoClase._idEstadoClase', 
                title: 'Estado', 
                type: 'select', 
                items: db.estadoClase, valueField: '_idEstadoClase', textField: '_nombre',
                sorting: false
            },
            {
                type: 'control',
                ...
                itemTemplate: function(value, item) {
                    return $('<button>').attr('type', 'button')
                            .attr('title', 'Estudiantes de la clase')
                            .button({icons: {primary: null}})
                            .addClass("buttonShowEstudiantes")
                            .on('click', function (e) { showEstudiantes(value, item); e.stopPropagation(); });
                },
                headerTemplate: function() {
                    return $('<button>').attr('type', 'button')
                            .attr('title', 'Agregar nueva clase')
                            .button({icons: {primary: null}})
                            .addClass("buttonAdd")
...
function configFormControls() {
    ...
    $.each(db.estadoClase, function (index, data) {
        if (index != 0)
            $("#sEstado").append("<option value='" + data._idEstadoClase + "'>" + data._nombre + "</option>");
    });
    ...
    $.ajax({
        ...
        data: { 
            ...
            f_nombreApellido: '',
            f_email: '',
            f_idTipoEmpleado: '1', //Tipo Profesor
...
function claseDuplicada(idClase, idDia, horaInicio, horaFin) {
    ...
    var clases = db.clases.filter( 
            clase => 
                clase._idClase != idClase && 
                clase._dia._idDia === idDia && 
                clase._estadoClase._idEstadoClase == '1' 
        );
